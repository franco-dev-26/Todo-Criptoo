...
function connect(){
  ...
  ws.onclose = ()=>{ statusBox.textContent = 'Reconectando…'; if(symbols.length) setTimeout(connect,1000); };
}
  ...
  el.innerHTML=`
    <div class="card__head">
      ...
      <div class="card__actions">
        <button data-act="fav" class="btn btn--ghost" title="Favorito">★</button>
...
function drawSpark(canvas, points, up){
  ...
  ctx.lineWidth = 2;
  ctx.strokeStyle = up ? '#16c172' : '#ef4444';
  ctx.beginPath();
  points.forEach((p,i)=>{
    const x=i*step;
...
function drawCandles(canvas, candles){
  ...
  candles.forEach((c,i)=>{
    ...
    const up = c.c >= c.o;
    ctx.strokeStyle = up ? '#16c172' : '#ef4444';
    ctx.fillStyle = up ? '#16c172' : '#ef4444';
    ctx.beginPath();
