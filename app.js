document.addEventListener('DOMContentLoaded', () => {
  ...
  async function tickMarket(){
    try{
      const u = 'https://data-api.binance.vision/api/v3/ticker/24hr'
      const r = await fetch(u); if(!r.ok) throw 0
      const arr = await r.json()
  ...
  function bindGeneralConverter(){
    ...
    const convert = (v, f, t) => {
      ...
      // fx.X = units of X per 1 USD
      // amount_in_to = (amount_in_from / fxFrom) * fxTo
    ...
    if(swap) swap.addEventListener('click', ()=>{
      const f = from.value; from.value = to.value; to.value = f; update()
    })
    update()
  }
