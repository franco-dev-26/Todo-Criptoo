...
async function loadCandles(sym){
  ...
  const url = `https://api.binance.com/api/v3/klines?symbol=${sym.toUpperCase()}&interval=15m&limit=96`;
...
function render(sym){
  ...
  chgEl.textContent = `${chg>=0?'▲':'▼'} ${chg.toFixed(2)}%`; chgEl.className=`badge ${chg>=0?'badge--up':'badge--down'}`;
...
async function loadFX(){
  ...
  try{ const r = await fetch('https://open.er-api.com/v6/latest/USD'); const j = await r.json();
    if(j && j.rates){ fxRates.EUR = Number(j.rates.EUR)||fxRates.EUR; fxRates.ARS = Number(j.rates.ARS)||fxRates.ARS; } }catch(e){}
  saveAll(); fxBadge.textContent = `FX · ARS ${fxRates.ARS.toFixed(2)} · EUR ${fxRates.EUR.toFixed(2)}`;
