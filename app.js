...
function connect(){
  ...
  ws.onmessage = (ev)=>{
    try{
      ...
      const spark = [...prev,price].slice(-140);
      state[sym] = { msg, spark };
      render(sym);
  ...
  ws.onclose = ()=>{ statusBox.textContent = 'Reconectando…'; if(symbols.length) setTimeout(connect,1200); };
}
  ...
  el.innerHTML=`
    <div class="card__head">
      ...
      <div class="card__actions">
        ...
        <button data-act="remove" class="card__remove">✕</button>
      </div>
    </div>
...
function render(sym){
  ...
  const rate = fxRates[fiat] || 1;
  const showPrice = fiat==='USD' ? priceUSDT : priceUSDT*rate;

  priceEl.textContent = `${fmtPrice(showPrice)} ${fiat}`;
