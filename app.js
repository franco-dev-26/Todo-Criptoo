...
async function loadDolaresAR(){
  try{
    ...
    console.log("Dólar API:", j);

    function pick(alias){
      alias = alias.toLowerCase();
      const d = j.find(x => ((x.casa||'').toLowerCase().includes(alias)) || ((x.nombre||'').toLowerCase().includes(alias)));
      const v = d?.venta ?? d?.valor_venta ?? d?.valor ?? d?.venta_promedio;
    ...
    const mep = pick('mep');
    const tarjeta = pick('tarjeta');

    const el = document.getElementById('fx-ar');
    if(el) el.textContent =
      `🇦🇷 Oficial $${oficial?nf.format(oficial):'—'} · Blue $${blue?nf.format(blue):'—'} · MEP $${mep?nf.format(mep):'—'} · Tarjeta $${tarjeta?nf.format(tarjeta):'—'}`;
  }catch(e){
    console.error("Error en loadDolaresAR:", e);
    const el = document.getElementById('fx-ar');
    if(el) el.textContent = '—';
  }
}
