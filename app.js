...
async function fetchJson(url, {timeout=9000, headers={}, ...opts} = {}){
  ...
  try{
    const res = await fetch(url, { method:'GET', mode:'cors', cache:'no-store', headers:{'accept':'application/json', ...headers}, signal:ctrl.signal, ...opts })
...
async function loadFX(){
  try{
    const r = await getRatesUSD()
    if(r){
      fx.EUR = r.EUR; fx.ARS = r.ARS; fx.CLP = r.CLP
      ...
      fxBox.textContent = 'Tasas no disponibles (offline)'
    }
    const usdIn = document.getElementById('fx-usd'), eurOut=document.getElementById('fx-eur'), arsOut=document.getElementById('fx-ars'), clpOut=document.getElementById('fx-clp')
    if(usdIn && eurOut && arsOut && clpOut){
      const upd=()=>{ const v=parseFloat(usdIn.value||'0'); eurOut.textContent=(v*fx.EUR).toFixed(2); arsOut.textContent=Math.round(v*fx.ARS).toLocaleString('es-AR'); clpOut.textContent=Math.round(v*fx.CLP).toLocaleString('es-AR') }
      usdIn.addEventListener('input', upd); upd()
    }
    const _amt=document.getElementById('conv-amount'); if(_amt){ _amt.dispatchEvent(new Event('input')) }
  ...
  function bindCalculators(); bindGeneralConverter(){
  ...
  try{
    ...
    ensureUI()
    bindCalculators(); bindGeneralConverter()
    loadFX()
    tick()
    startPolling()
    tickMarket()
